  ...
  "bank-db-store-crud": {
    ...
    blocks: {
      th: [
        ...
        {
          t: "callout",
          title: "📁 โครงสร้างแพ็กเกจของโปรเจกต์ Simple Bank",
          c: "ตั้งแต่บทนี้เป็นต้นไป เรากำลังเขียนโค้ดลงในโครงสร้างโปรเจกต์จริง โดยโค้ดในโฟลเดอร์ `db/` ทั้งหมดจะถูกประกาศเป็น `package db` (เช่น `db/db.go`, `db/models.go`, `db/account.go`, `db/store.go`) ซึ่งทำหน้าที่เป็น **Data Access Layer** สำหรับทั้งระบบ และจะถูกนำไปเรียกใช้งานพร้อมกันผ่านชุดทดสอบอัตโนมัติ (`go test ./db`) ในบทที่ 5 และผ่าน Web API Server ในบทที่ 6!",
        },
