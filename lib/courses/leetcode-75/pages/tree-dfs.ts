...
export const treeDfsPages: Record<string, Page> = {
  "lc75-intro-tree-dfs": {
    ...
    lead: "รู้จัก binary tree ตั้งแต่ศูนย์ — node, ใบ, ราก, คลาส TreeNode — แล้วเรียนวิธีลุยลงลึกทีละกิ่งด้วย recursion (DFS)",
    group: "LeetCode 75",
    blocks: [
      { t: "p", c: "จนถึงตอนนี้เราเล่นกับข้อมูลที่เรียงเป็นแถวยาว ๆ (ลิสต์, สตริง, linked list) มาตลอด หมวดนี้เราจะเจอโครงสร้างข้อมูลแบบใหม่ที่แตกกิ่งก้านเหมือนต้นไม้ ชื่อว่า binary tree มันโผล่ในโจทย์สัมภาษณ์บ่อยมาก และเครื่องมือหลักที่ใช้ตะลุยมันคือ recursion (DFS) ถ้ายังไม่เคยรู้จัก tree เลยก็ไม่เป็นไร หน้านี้เราจะเริ่มจากศูนย์แล้วค่อยไปลุยโจทย์ทีละข้อในหน้าถัดไป" },
...
# 9 ไม่มีลูก จึงเป็น leaf` },
      ...
      { t: "p", c: "LeetCode สร้าง node แต่ละตัวจากคลาส TreeNode หน้าตาแบบนี้ ตัวมันเองเก็บค่า val และตัวชี้ไปหาลูกสองตัวคือ left กับ right ถ้าไม่มีลูกฝั่งไหน ตัวชี้ฝั่งนั้นจะเป็น None โจทย์ tree ทุกข้อในหมวดนี้อิงหน้าตานี้" },
...
# root.right.left.val == 15` },
      ...
      { t: "p", c: "DFS ย่อมาจาก Depth-First Search แปลตรงตัวว่า \"ค้นแบบลงลึกก่อน\" ไอเดียคือเลือกกิ่งหนึ่งแล้วเดินดิ่งลงไปให้สุดก่อน จนชนใบ แล้วค่อยถอยกลับมาลองกิ่งอื่น ต่างจากการไล่ทีละชั้น (นั่นคือ BFS ซึ่งเราจะเรียนหมวดถัดไป)" },
...
def dfs(node):
    ...
    return combine(node.val, left, right)` },
      ...
      { t: "p", c: "ในโจทย์ส่วนใหญ่ของหมวดนี้เราใช้แนว postorder คือ \"ให้ลูกสองฝั่งคำนวณเสร็จก่อน แล้วค่อยเอาผลมารวมที่ตัวเอง\" เพราะคำตอบของ node หนึ่งมักต้องพึ่งคำตอบของลูกทั้งสอง แต่บางข้อ (เช่นนับ good node) ใช้แนว preorder ที่ \"ส่งข้อมูลลงล่าง\" แทน" },
  ...
  "lc75-p33": {
    slug: "lc75-p33",
    title: "ข้อ 33 · LC104 — ความลึกมากสุดของต้นไม้ 🟢",
    lead: "หาความลึกมากสุดของ binary tree ด้วย recursion แบบ postorder — โจทย์เปิดตัวของ DFS บนต้นไม้",
    group: "LeetCode 75",
    blocks: [
      { t: "p", c: "โจทย์: ให้ root ของ binary tree มา ให้หาความลึกมากสุด = จำนวน node บนเส้นทางที่ยาวสุดจาก root ลงไปถึง leaf" },
      { t: "ul", c: [
      ...
      { t: "p", c: "ใช้ DFS recursion แบบ postorder เพราะความลึกของต้นหนึ่งขึ้นกับความลึกของลูกทั้งสองฝั่ง เราจึงต้องรอให้ลูกคำนวณเสร็จก่อนแล้วค่อยสรุปตัวเอง" },
      { t: "p", c: "ถ้าคิดตรง ๆ เราอาจลองไล่ทุกเส้นทางจาก root ถึงทุก leaf แล้วหาเส้นที่ยาวสุด แต่การมองเป็น recursion ทำให้สั้นกว่ามาก: ความลึกของต้น = 1 (นับตัว root เอง) บวกกับความลึกที่มากกว่าของสองต้นย่อย ปล่อยให้ลูกซ้ายและลูกขวาบอกความลึกของมันมาเอง" },
      { t: "ol", c: [
        "ถ้า node เป็น None แปลว่าต้นว่าง คืน 0 (base case)",
        "ถามความลึกของต้นย่อยซ้าย เก็บไว้ในตัวแปร left",
        "ถามความลึกของต้นย่อยขวา เก็บไว้ในตัวแปร right",
        "คืน 1 + max(left, right) — บวก 1 นับตัวเอง เลือกฝั่งที่ลึกกว่า",
      ] },
      { t: "callout", title: "จุดพลาดที่พบบ่อย", c: "ลืม base case node is None → จะเรียก .left / .right ทะลุ None แล้ว error ทันที และอย่าลืมว่านับ \"จำนวน node\" ไม่ใช่จำนวนเส้นเชื่อม จึงต้องบวก 1 ทุกชั้น" },
...
def maxDepth(root):
  ...
  "lc75-p34": {
    slug: "lc75-p34",
    title: "ข้อ 34 · LC872 — ต้นไม้ใบเหมือนกัน 🟢",
    lead: "เทียบว่าลำดับ leaf จากซ้ายไปขวาของต้นไม้สองต้นเหมือนกันไหม ด้วย DFS เก็บใบ",
    group: "LeetCode 75",
    blocks: [
      { t: "p", c: "โจทย์: ให้ root ของต้นไม้สองต้น ให้ตอบว่าลำดับของ \"ค่าที่ leaf จากซ้ายไปขวา\" ของสองต้นเหมือนกันหรือไม่ (สนแค่ใบ ไม่สนโครงหรือ node ภายใน)" },
      { t: "ul", c: [
      ...
      { t: "h2", c: "แนวทาง — ต้องใช้อะไร & คิดยังไง" },
      { t: "p", c: "ใช้ DFS เก็บค่าเฉพาะ node ที่เป็น leaf ออกมาเป็นลิสต์ของแต่ละต้น แล้วเทียบสองลิสต์ว่าเท่ากันไหม" },
      { t: "p", c: "เคล็ดคือทำให้ลำดับ \"ซ้ายไปขวา\" เป๊ะ ๆ: การเดินลูกซ้ายก่อนลูกขวาเสมอ (แล้วต่อลิสต์ผลซ้าย + ผลขวา) จะรับประกันลำดับใบจากซ้ายไปขวาโดยอัตโนมัติ" },
      { t: "ol", c: [
        "เขียนฟังก์ชันย่อย leaves(node) ที่คืนลิสต์ค่าใบของต้นนั้น",
        "ถ้า node เป็น None คืนลิสต์ว่าง []",
        "ถ้า node ไม่มีลูกทั้งซ้ายและขวา = เป็น leaf คืน [node.val]",
        "ไม่งั้นคืน leaves(ซ้าย) + leaves(ขวา) — ต่อฝั่งซ้ายก่อนเสมอ",
  ...
  "lc75-p35": {
    slug: "lc75-p35",
    title: "ข้อ 35 · LC1448 — นับโหนดดี 🟡",
    lead: "นับ node ที่ไม่มีใครบนเส้นทางจาก root ค่ามากกว่ามัน ด้วย DFS แบบ top-down พกค่ามากสุดลงไป",
    group: "LeetCode 75",
    blocks: [
      { t: "p", c: "โจทย์: node หนึ่งถือว่า \"ดี\" (good) ถ้าบนเส้นทางจาก root ลงมาถึง node นั้น ไม่มี node ไหนที่ค่ามากกว่ามันเลย (พูดอีกแบบคือค่าของมันมากกว่าหรือเท่ากับทุก node ที่ผ่านมาก่อนหน้าบนเส้นทางนั้น) ให้นับว่ามี good node กี่ตัว หมายเหตุ: root เป็น good เสมอ เพราะไม่มีใครอยู่ก่อนหน้ามัน" },
      { t: "ul", c: [
      ...
      { t: "h2", c: "แนวทาง — ต้องใช้อะไร & คิดยังไง" },
      { t: "p", c: "ใช้ DFS แบบ preorder ที่ \"ส่งข้อมูลลงล่าง\" (top-down): ระหว่างเดินลงจาก root ให้พก \"ค่ามากสุดที่เจอมาตลอดเส้นทาง\" ติดตัวไปด้วย" },
      { t: "p", c: "ต่างจากข้อ 33-34 ที่รอผลจากลูกขึ้นมา (bottom-up) ข้อนี้เราตัดสินที่ node ปัจจุบันก่อน (ดูว่าค่ามันมากกว่าหรือเท่ากับ max ที่พกมาไหม) แล้วค่อยส่ง max ที่อัปเดตแล้วลงให้ลูก" },
      { t: "ol", c: [
        "เขียน dfs(node, max_so_far) โดย max_so_far คือค่ามากสุดบนเส้นทางถึงก่อน node นี้",
        "ถ้า node เป็น None คืน 0",
        "node นี้ดี (นับ 1) ถ้า node.val >= max_so_far ไม่งั้นนับ 0",
        "คำนวณ new_max = max(max_so_far, node.val) ก่อนส่งต่อ",
        "คืน good + dfs(ซ้าย, new_max) + dfs(ขวา, new_max)",
  ...
  "lc75-p36": {
    slug: "lc75-p36",
    title: "ข้อ 36 · LC437 — นับ path ผลรวมเท่าเป้า 🟡",
    lead: "นับเส้นทางลงล่างที่ผลรวมเท่า targetSum ด้วยเทคนิค prefix sum + hash map ระหว่างเดิน DFS",
    group: "LeetCode 75",
    blocks: [
      { t: "p", c: "โจทย์: ให้ต้นไม้และเลข targetSum ให้นับจำนวนเส้นทางที่ผลรวมของค่าบนเส้นทางเท่ากับ targetSum โดยเส้นทางต้องเดินลงล่างเท่านั้น (จากแม่ไปหาลูก) และต่อเนื่องกัน แต่ไม่จำเป็นต้องเริ่มที่ root หรือจบที่ leaf (ค่าติดลบก็มีได้)" },
      { t: "ul", c: [
      ...
      { t: "h2", c: "แนวทาง — ต้องใช้อะไร & คิดยังไง" },
      { t: "p", c: "ใช้ prefix sum (ผลรวมสะสมจาก root ลงมาถึง node ปัจจุบัน) เก็บความถี่ไว้ใน hash map ระหว่างเดิน DFS — เป็นเทคนิคเดียวกับ prefix sum บนลิสต์ แต่ทำบนเส้นทางของต้นไม้" },
...
def pathSum(root, targetSum):
  ...
  "lc75-p37": {
    slug: "lc75-p37",
    title: "ข้อ 37 · LC1372 — ทางซิกแซกยาวสุด 🟡",
    lead: "หาความยาวเส้นทางที่สลับซ้าย-ขวาทุกก้าว ด้วย DFS ที่พกทิศและความยาวสะสม",
    group: "LeetCode 75",
    blocks: [
      { t: "p", c: "โจทย์: เส้นทางซิกแซก (ZigZag) คือเดินจาก node หนึ่ง โดยสลับทิศทุกก้าว: ถ้าก้าวนี้ไปลูกซ้าย ก้าวหน้าต้องไปลูกขวา แล้วซ้าย สลับไปเรื่อย ๆ ให้หาความยาว (นับเป็นจำนวนก้าว/เส้นเชื่อม ไม่ใช่จำนวน node) ของเส้นทางซิกแซกที่ยาวสุดในต้นไม้" },
      { t: "ul", c: [
      ...
      { t: "h2", c: "แนวทาง — ต้องใช้อะไร & คิดยังไง" },
      { t: "p", c: "ใช้ DFS ที่แต่ละ node พก 2 อย่าง: ทิศที่กำลังจะไปต่อ (go_left) กับ ความยาวสะสมมาถึงตอนนี้ ถ้าเดินตามทิศที่ควรไปก็ +1 ต่อความยาว ถ้าเดินอีกทางก็ถือว่าเริ่มซิกแซกเส้นใหม่ที่ความยาว 1" },
      { t: "ol", c: [
      ...
      { t: "callout", title: "จุดพลาดที่พบบ่อย", c: "สับสนว่านับ node หรือนับเส้นเชื่อม — โจทย์นับเส้นเชื่อม (ก้าว) จึงเริ่มที่ length 0 ที่ root และต้องอัปเดต ans ที่ทุก node ที่แวะ (ไม่ใช่เฉพาะตอนจบ) เพราะเส้นซิกแซกที่ดีสุดอาจจบกลางต้นไม้ก็ได้" },
  ...
  "lc75-p38": {
    slug: "lc75-p38",
    title: "ข้อ 38 · LC236 — บรรพบุรุษร่วมต่ำสุด 🟡",
    lead: "หา lowest common ancestor ของสอง node ด้วย DFS แบบ postorder ที่ให้ลูกรายงานขึ้นมา",
    group: "LeetCode 75",
    blocks: [
      { t: "p", c: "โจทย์: ให้ root และ node สองตัว p, q (รับประกันว่าอยู่ในต้นไม้ทั้งคู่) ให้หา lowest common ancestor (LCA) = node ที่เป็นบรรพบุรุษร่วมของทั้ง p และ q ที่อยู่ต่ำสุด (ลึกสุด) โดยนิยามให้ node หนึ่งเป็นบรรพบุรุษของตัวเองได้ ค่าใน node ทุกตัวไม่ซ้ำกัน" },
      { t: "ul", c: [
      ...
      { t: "h2", c: "แนวทาง — ต้องใช้อะไร & คิดยังไง" },
      { t: "p", c: "ใช้ DFS แบบ postorder: เดินค้นหา p และ q ถ้า node ปัจจุบันเจอ p (หรือ q) ให้รายงานตัวเองขึ้นไป จากนั้นถ้า node ไหนได้รับรายงานว่า \"เจอของอยู่ทั้งฝั่งซ้ายและฝั่งขวา\" node นั้นแหละคือจุดที่สองคนมาบรรจบกัน = LCA" },
      { t: "ol", c: [
        "base case: ถ้า root เป็น None หรือ root คือ p หรือ q ให้คืน root ขึ้นไป (สัญญาณ \"เจอที่นี่\")",
        "เรียก recursion ฝั่งซ้าย เก็บผลไว้ในตัวแปร left",
        "เรียก recursion ฝั่งขวา เก็บผลไว้ในตัวแปร right",
        "ถ้า left และ right ต่างก็ไม่ None (เจอทั้งสองฝั่ง) → node นี้คือ LCA คืน root",
        "ไม่งั้นคืนฝั่งที่เจอ (left ถ้ามี ไม่งั้น right) ส่งต่อขึ้นไป",
      ] },
