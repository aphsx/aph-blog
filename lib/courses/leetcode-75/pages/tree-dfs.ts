...

        {
          ...
          c: [
            ...
            { t: "h3", c: "1. Pattern Recognition (วิธีมองโจทย์ให้ออก)" },
            {
              t: "ul",
              c: [
                "**บรรพบุรุษร่วม (Common Ancestor):** โหนดที่มีทั้ง p และ q เป็นลูก/หลานอยู่ใน subtree ของมัน",
                "**ต่ำที่สุด (Lowest):** บรรพบุรุษร่วมที่อยู่ลึกที่สุด (ห่างจาก root มากที่สุด)",
                "**โหนดเป็นบรรพบุรุษของตัวเองได้:** โหนด p เป็น LCA ของ p และ q ได้ ถ้า q อยู่ใต้ p",
              ],
            },

            { t: "h3", c: "2. เครื่องมือที่เลือกใช้ & กลไกการคืนค่า (Return Contract)" },
            {
              t: "p",
              c: "**DFS (Post-Order Traversal):** สำรวจกิ่งซ้ายและขวาก่อน (left → right → root) เพื่อให้โหนดลูกส่งข้อมูลกลับขึ้นมาบอกโหนดพ่อ",
            },
            {
              t: "p",
              c: "ฟังก์ชัน DFS คืนค่ากลับขึ้นมาได้ **1 ใน 3 อย่าง**:",
            },
            {
              t: "ul",
              c: [
                "**คืน None** → ใน subtree ฝั่งนี้ไม่พบทั้ง p และ q",
                "**คืนโหนด (p หรือ q)** → ใน subtree ฝั่งนี้พบตัวใดตัวหนึ่ง",
                "**คืนโหนด (LCA)** → พบ LCA แล้ว ส่งโหนดนี้ผ่านขึ้นบนเป็นคำตอบได้เลย",
              ],
            },
            ...
            {
              t: "p",
              c: "เมื่อ DFS ถอยกลับขึ้นมาจากกิ่งซ้าย (left) และกิ่งขวา (right) โหนดปัจจุบันเจอเพียง 3 กรณีหลัก:",
            },
            {
              t: "code",
              lang: "text",
           ...
           /          \\
          /            \\
 ผลจากซ้าย (left)   ผลจากขวา (right)`,
            },
            {
              t: "ol",
              c: [
                "**กรณีที่ 1: เจอทั้งซ้ายและขวา** (left และ right ไม่เป็น None) → p อยู่ฝั่งหนึ่ง q อยู่อีกฝั่งหนึ่ง → **โหนดปัจจุบันคือ LCA** คืน node ปัจจุบันกลับขึ้นไป",
                "**กรณีที่ 2: เจอแค่ฝั่งเดียว** (left มีค่า หรือ right มีค่า) → ทั้ง p และ q อยู่ subtree ฝั่งเดียวกัน → ส่งต่อโหนดที่เจอขึ้นไปหาพ่อข้างบน",
                "**กรณีที่ 3: ไม่เจอทั้งสองฝั่ง** (left เป็น None และ right เป็น None) → คืน None กลับขึ้นไป",
              ],
            },
            ...
            { t: "h3", c: "4. ชำแหละไวยากรณ์และตรรกะโค้ด (Line-by-Line Breakdown)" },
            {
              t: "code",
              lang: "python",
              label: "Base Case: ตกขอบ หรือเจอ p หรือเจอ q",
              c: `if not root or root == p or root == q:
    return root`,
            },
            {
              t: "callout",
              title: "ชำแหละ Base Case",
              c: "`if not root` → เดินตกขอบต้นไม้ คืนค่า None; `if root == p or root == q` → เจอเป้าหมาย! คืนโหนดนั้นกลับขึ้นไปทันที ไม่ต้องลงไปหาใต้โหนดนี้ต่อ (เพราะถ้าตัวที่เหลืออยู่ใต้โหนดนี้ โหนดนี้ก็เป็น LCA เองตามกฎ)",
            },
            {
              t: "code",
              lang: "python",
              label: "ท่อง DFS ลงกิ่งซ้ายและขวา (Post-Order)",
...
right = self.lowestCommonAncestor(root.right, p, q)`,
            },
            {
              t: "code",
              lang: "python",
              label: "กรณีที่ 1: เจอทั้งซ้ายและขวา → โหนดปัจจุบันคือ LCA",
              c: `if left and right:
    return root`,
            },
            {
              t: "callout",
              title: "ชำแหละกรณีที่ 1",
              c: "ฝั่งซ้ายเจอตัวหนึ่ง ฝั่งขวาเจออีกตัวหนึ่ง จุดบรรจบกันจุดแรกคือ root ปัจจุบัน จึงคืน root เป็น LCA",
            },
            {
              t: "code",
              lang: "python",
              label: "กรณีที่ 2 & 3: คืนค่าฝั่งที่มีข้อมูลกลับขึ้นไป",
              c: `return left if left else right`,
            },
            {
              t: "callout",
              title: "ชำแหละ `return left if left else right`",
              c: "ถ้า left มีค่า (ฝั่งขวาเป็น None) → คืน left; ถ้า left เป็น None แต่ right มีค่า → คืน right; ถ้าเป็น None ทั้งคู่ → คืน right (ซึ่งก็คือ None)",
            },
    ...
    6   2`,
            ...
            {
              t: "p",
              c: "หา LCA ของ **p = 6** และ **q = 2**",
            },
            {
              t: "ol",
              c: [
                "**DFS เข้า Node 3:** ส่งไปสำรวจซ้าย (Node 5) และขวา (Node 1)",
                "**เข้า Node 5:** ส่งไปสำรวจซ้าย (Node 6) และขวา (Node 2)",
                "**เข้า Node 6:** `root == p` (6 == 6) → คืน Node 6 กลับขึ้นไป",
                "**เข้า Node 2:** `root == q` (2 == 2) → คืน Node 2 กลับขึ้นไป",
                "**กลับมาที่ Node 5:** ได้ left = Node 6, right = Node 2 → `if left and right:` เป็นจริง → Node 5 คืนตัวเอง (LCA) กลับขึ้นไป",
                "**เข้า Node 1:** ไม่พบ p หรือ q → คืน None",
                "**กลับมาที่ Node 3 (Root):** ได้ left = Node 5 (คือ LCA), right = None → `return left if left else right` → คืน Node 5 เป็นคำตอบสุดท้าย",
              ],
            },

            { t: "h3", c: "6. Clean Code (โค้ดฉบับสมบูรณ์)" },
            {
              t: "code",
              lang: "python",
              label: "คำตอบสำหรับวางใน LeetCode",
...
class Solution:
    def lowestCommonAncestor(self, root: 'TreeNode', p: 'TreeNode', q: 'TreeNode') -> 'TreeNode':
        # Base Case: ถ้าเดินตกขอบ (None) หรือเจอ p หรือเจอ q ให้คืนค่าโหนดนั้นกลับขึ้นไป
        if not root or root == p or root == q:
            return root

        # 1. ให้กิ่งซ้ายไปหา p และ q
        left = self.lowestCommonAncestor(root.left, p, q)

        # 2. ให้กิ่งขวาไปหา p และ q
        right = self.lowestCommonAncestor(root.right, p, q)

        # 3. ถ้าเจอทั้งกิ่งซ้ายและกิ่งขวา แปลว่าโหนดปัจจุบันคือ LCA
        if left and right:
            return root

        # 4. ถ้าเจอแค่ฝั่งเดียว ให้ส่งฝั่งที่เจอขึ้นไปต่อ (ถ้าไม่เจอทั้งสองฝั่งจะคืน None)
        return left if left else right`,
            ...
            { t: "h3", c: "7. Complexity Analysis (วิเคราะห์ประสิทธิภาพ)" },
            {
              t: "ul",
              c: [
                "**Time Complexity: O(N)** — กรณีแย่สุดเยี่ยมชมทุกโหนด N โหนด อย่างมากโหนดละ 1 ครั้ง",
                "**Space Complexity: O(N)** — ใช้ Call Stack ของ recursion เท่ากับความสูงของต้นไม้ กรณีแย่สุด (ต้นไม้เบี้ยวเป็นเส้นตรง) คือ O(N)",
              ],
            },
