...
return get_leaves(node.left) + get_leaves(node.right)`,
    ...
    6   2`,
            ...
            {
              t: "table",
              head: ["ขั้นตอน", "เกิดอะไรขึ้น", "กล่องผลลัพธ์"],
              rows: [
                [
                  "จุดเริ่มต้น (Root): Node 3",
                  'ไม่ใช่ทางตัน → ฟังก์ชันทำงานสถานการณ์ที่ 3: สั่ง "ไปเอากล่องซ้ายมาต่อกับกล่องขวานะ" → ดำดิ่งไปที่ node.left (ไปที่ 5)',
                  "—",
                ],
                ["สำรวจกิ่งซ้าย: Node 5", "ไม่ใช่ทางตัน → ดำดิ่งไปซ้ายต่อ (ไปที่ 6)", "—"],
                [
                  "เจอ Leaf ตัวแรก!: Node 6",
                  "สถานการณ์ที่ 2 (ไม่มีลูกซ้ายและขวา) → เอาตัวเองใส่กล่อง แล้ว Return กล่องนี้ย้อนกลับขึ้นไปให้ Node 5",
                  "[6]",
                ],
                [
                  "เจอ Leaf ตัวที่สอง!: Node 2",
                  "Node 5 ได้กล่องซ้ายมาแล้ว → ไปสำรวจกิ่งขวาต่อ → เข้าเงื่อนไขสถานการณ์ที่ 2 เช่นกัน → ใส่กล่อง แล้ว Return กลับไปให้ Node 5",
                  "[2]",
                ],
                [
                  "รวมกล่องที่ทางแยก: Node 5",
                  "ได้กล่องครบแล้ว → เอามาต่อกันตามลอจิก ซ้าย + ขวา แล้ว Return กล่องใหญ่นี้ย้อนกลับไปให้ Root 3",
                  "[6] + [2] = [6, 2]",
                ],
                [
                  "สำรวจกิ่งขวาของ Root: Node 1",
                  "Root 3 ได้กล่องซ้ายมาแล้ว → ลงไปสำรวจกิ่งขวา → เจอ Node 1 เข้าเงื่อนไขสถานการณ์ที่ 2 → ใส่กล่อง แล้ว Return ย้อนกลับไปให้ Root 3",
                  "[1]",
                ],
                ["บทสรุป: Node 3", "เอากล่องซ้ายและขวามาต่อกัน: [6, 2] + [1]", "[6, 2, 1]"],
              ],
            },
...
โจทย์ต้องการให้เราหาว่ามี "เส้นทาง (Path)" ทั้งหมดกี่เส้นที่ตัวเลขในโหนดบวกกันแล้วได้เท่ากับ targetSum พอดี`,
            ...
            {
              t: "p",
              c: 'ให้มองว่าคุณคือ "นักปีนเขาเก็บแต้ม" ที่เดินลงเขาไปเรื่อยๆ การจะมานั่งนับแต้มใหม่ทุกครั้งจากทุกๆ จุดเริ่มต้น มันเหนื่อยและเสียเวลา (Brute Force) เราจึงใช้เทคนิค "สมุดบันทึกยอดสะสม (Prefix Sum Notebook)" เข้ามาช่วยครับ',
            },
            {
              t: "p",
            ...
            },
            {
              t: "ul",
              c: [
                'ระหว่างที่คุณเดินลงเขา คุณจะจด "แต้มรวมทั้งหมด (Current Sum)" ตั้งแต่ยอดเขาลงในสมุดเสมอ',
                "สมมติว่าเป้าหมายแต้มที่อยากได้คือ 8 และตอนนี้คุณเดินมาถึงจุดที่แต้มรวมคือ 15",
                "คุณอยากรู้ว่าระหว่างทางมีช่วงไหนไหมที่รวมกันได้ 8 พอดี?",
                'แทนที่จะเดินย้อนกลับไปนับ คุณแค่เปิดสมุดบันทึกแล้วดูว่า "ก่อนหน้านี้... เราเคยจดแต้มสะสมที่ 7 แต้มไว้หรือเปล่า?"',
                "ทำไมต้องเป็นเลข 7? เพราะ 15 (ยอดปัจจุบัน) - 8 (ยอดเป้าหมาย) = 7 (ยอดในอดีต)",
                "ถ้าในสมุดมีเลข 7 อยู่ 1 ครั้ง แปลว่าตั้งแต่จุดที่คุณได้ 7 แต้ม เดินต่อมาถึงปัจจุบัน คุณเก็บแต้มเพิ่มมาได้ 8 แต้มพอดี!",
              ],
            },
            ...
            {
              t: "p",
              c: 'เราจะสร้างฟังก์ชัน DFS เดินลุยลงไปในต้นไม้ โดยนี่คือหัวใจของโค้ด 4 ส่วนพร้อมเหตุผลที่คุณต้องตอบให้ได้ว่า "ทำไมถึงเขียนแบบนี้?"',
            },
            ...
            },
            {
              ...
              warn: true,
              c: 'เส้นทางที่บวกกันได้ targetSum พอดีตั้งแต่ Root (เช่น [8], [3, 5]) จะถูกข้ามไปทั้งหมด เพราะหาเลข 0 ในสมุดไม่เจอ คำตอบจะขาดหายไปทันที',
            },
...
paths = prefix_map.get(old_sum, 0)`,
            },
            {
              t: "callout",
              title: "ทำไมต้องเขียน (The Why)",
              c: "นี่คือการพลิกมุมมองแทนที่จะเอา targetSum ไล่ลบย้อนกลับขึ้นไป เราใช้พีชคณิตง่ายๆ: ถ้ายอดรวม = 15, เป้าหมาย = 8 แสดงว่า X + 8 = 15 ดังนั้นค่าในอดีต X = 15 - 8 = 7 เราแค่เช็คว่ามีเลข 7 ในสมุดไหม",
            },
            {
              t: "callout",
              title: "ถ้าไม่เขียน (What If)",
              warn: true,
              c: "ถ้าคุณพยายามใช้ลูปไล่บวกย้อนกลับไปหาพ่อ (Brute Force) Time Complexity จะพังจาก O(N) กลายเป็น O(N²) ทันที",
            },
...
paths += dfs(node.right, current_sum)`,
            },
            {
              t: "callout",
              title: "ทำไมต้องเขียน (The Why)",
              c: 'เราต้องเอาผลรวมปัจจุบันจดลงสมุด (บวกความถี่เพิ่ม 1) เพื่อให้ "ลูกๆ และหลานๆ" ที่กำลังจะเดินไปสำรวจต่อ สามารถมองเห็นยอดสะสมนี้ได้ และเอาคำตอบจากกิ่งซ้าย-ขวามาบวกทบกัน',
            },
            ...
            },
            {
              ...
              title: "ถ้าไม่เขียน (What If)",
              warn: true,
              c: 'พังพินาศครับ! กิ่งทางซ้ายกับกิ่งทางขวาคือ "เส้นทางคู่ขนาน" ถ้าคุณไม่ลบรอยเท้าของกิ่งซ้ายทิ้ง พอกระโดดไปกิ่งขวา กิ่งขวาจะเห็นข้อมูลของกิ่งซ้าย และเอามาคำนวณเชื่อมกัน ซึ่งแหกกฎโจทย์ที่ว่า "ต้องวิ่งลงข้างล่างเสมอ" ครับ',
            },
  ...
  3   2    11`,
            ...
            { t: "h3", c: "4. ถอยหลัง (Backtrack)" },
            {
              ...
              c: [
                ...
                "สมุดจะกลับมาเป็น {0: 1, 10: 1, 15: 1} เพื่อให้โหนด 5 พร้อมส่งสมุดที่สะอาดๆ ไปให้ลูกกิ่งขวา (โหนด 2) ต่อไป",
              ],
            },
