...
export const dp1dPages: Record<string, Page> = {
  "lc75-intro-dp-1d": {
    ...
    title: { th: "Dynamic Programming 1 มิติ — พื้นฐาน & แนวคิด", en: "" },
    lead: { th: "แก้ปัญหาใหญ่ด้วยการต่อยอดคำตอบของปัญหาย่อยที่ซ้ำกัน แล้วเก็บไว้ใช้ซ้ำแทนที่จะคำนวณใหม่", en: "" },
    group: "LeetCode 75",
    blocks: {
      th: [
...
print(fib(10))  # 55` },
  ...
  "lc75-p59": {
    slug: "lc75-p59",
    title: { th: "ข้อ 59 · LC1137 N-th Tribonacci Number (เลข Tribonacci ตัวที่ n) 🟢", en: "" },
    lead: { th: "เหมือน Fibonacci แต่ sum สามตัวก่อนหน้า ฝึกยุบ DP table ให้เหลือ Space O(1)", en: "" },
    group: "LeetCode 75",
    blocks: {
      th: [
              { t: "p", c: "โจทย์ (LC1137): Tribonacci sequence นิยามว่า T0 = 0, T1 = 1, T2 = 1 และ Tn+3 = Tn + Tn+1 + Tn+2 สำหรับ n >= 0 กำหนดจำนวนเต็ม n มา ให้ return ค่า Tn" },
              {
                t: "example",
                c: [
                  {
                  ...
                  },
                  {
                    input: "n = 0",
                    output: "0",
                    explain: "base case แยกจาก n = 1 และ 2 ซึ่งตอบ 1 ทั้งคู่",
                  },
                ],
              },
              {
...
1389537` },
  ...
  "lc75-p60": {
    slug: "lc75-p60",
    title: { th: "ข้อ 60 · LC746 Min Cost Climbing Stairs (ขึ้นบันไดถูกสุด) 🟢", en: "" },
    lead: { th: "choose ก้าว 1 หรือ 2 ขั้นให้พ้นบันไดโดยจ่าย cost รวม minimum (น้อยที่สุด) ด้วย DP แบบ Fibonacci", en: "" },
    group: "LeetCode 75",
    blocks: {
      th: [
              { t: "p", c: "โจทย์ (LC746): กำหนด array จำนวนเต็ม cost มา โดย cost[i] คือค่าใช้จ่ายของขั้นบันไดที่ i เมื่อจ่ายค่าใช้จ่ายของขั้นนั้นแล้ว สามารถก้าวขึ้นได้ 1 หรือ 2 ขั้น สามารถเริ่มต้นได้จากขั้นที่ index 0 หรือ index 1 ก็ได้ ให้หาค่าใช้จ่าย minimum ที่จะไปให้ถึงยอดบันได (จุดที่พ้นขั้นสุดท้าย)" },
              {
                t: "example",
                c: [
                  {
                    input: "cost = [10, 15, 20]",
                    output: "15",
                    explain: "เริ่มที่ขั้น index 1 จ่าย 15 แล้วก้าว 2 ขั้นพ้นยอดบันไดพอดี",
                  },
                  {
                    input: "cost = [1, 100, 1, 1, 1, 100, 1, 1, 100, 1]",
                    output: "6",
                    explain: "เริ่มที่ขั้น index 0 แล้วก้าวทีละ 2 ขั้นไปเรื่อย ๆ (0→2→4→6) จ่าย 1 บาทสี่ครั้ง จากนั้นก้าว 1 ขั้นไป index 7 (จ่าย 1 บาท) แล้วก้าว 2 ขั้นพ้นยอด (จ่าย 1 บาท) รวมเป็น 6",
                  },
                ],
              },
              {
...
6` },
  ...
  "lc75-p61": {
    slug: "lc75-p61",
    title: { th: "ข้อ 61 · LC198 House Robber (ขโมยบ้าน) 🟡", en: "" },
    lead: { th: "choose ขโมยบ้านให้ได้เงิน maximum โดยห้ามขโมยสองหลัง adjacent (ติดกัน) ด้วย DP แบบ take/skip", en: "" },
    group: "LeetCode 75",
    blocks: {
      th: [
              { t: "p", c: "โจทย์ (LC198): วางแผนขโมยบ้านที่เรียงกันเป็นแถว แต่ละหลังมีเงินสดเก็บอยู่จำนวนหนึ่ง ข้อจำกัดเดียวคือบ้านที่ adjacent (ติดกัน) มีระบบรักษาความปลอดภัยเชื่อมถึงกัน ถ้าขโมยสองหลังที่ adjacent ในคืนเดียวกัน ระบบจะแจ้งตำรวจโดยอัตโนมัติ กำหนด array จำนวนเต็ม nums ซึ่ง nums[i] คือเงินสดในบ้านหลังที่ i มา ให้หาจำนวนเงิน maximum ที่ขโมยได้โดยไม่โดนแจ้งเตือน" },
              {
                t: "example",
                c: [
                  {
                    input: "nums = [1, 2, 3, 1]",
                    output: "4",
                    explain: "ขโมยหลัง index 0 (เงิน 1) และหลัง index 2 (เงิน 3) รวม 1 + 3 = 4",
                  },
                  {
                    input: "nums = [2, 7, 9, 3, 1]",
                    output: "12",
                    explain: "ขโมยหลัง index 0 (เงิน 2), index 2 (เงิน 9) และ index 4 (เงิน 1) รวม 2 + 9 + 1 = 12",
                  },
                  {
                    input: "nums = [2, 1, 1, 2]",
                    output: "4",
                    explain: "ขโมยหลังแรกกับหลังสุดท้าย (เว้นสองหลังตรงกลาง) ได้ 2 + 2 = 4",
                  },
                ],
              },
              {
...
12` },
  ...
  "lc75-p62": {
    slug: "lc75-p62",
    title: { th: "ข้อ 62 · LC790 Domino and Tromino Tiling (ปูกระเบื้องโดมิโน) 🟡", en: "" },
    lead: { th: "count (นับ) จำนวนวิธีปู board 2xn ด้วย domino และ tromino ด้วย DP recurrence และการ mod เลขใหญ่", en: "" },
    group: "LeetCode 75",
    blocks: {
      th: [
              { t: "p", c: "โจทย์ (LC790): มี tile (กระเบื้อง) สองแบบ ได้แก่ domino (โดมิโน) ขนาด 2 x 1 และ tromino (โทรมิโน) รูปตัว L ซึ่งทั้งสองแบบ rotate (หมุน) ได้ กำหนดจำนวนเต็ม n มา ให้หาจำนวนวิธีปู board (กระดาน) ขนาด 2 x n ทั้งหมด (คำตอบอาจมีขนาดใหญ่มาก ให้ตอบเป็นเศษจากการหารด้วย 10^9 + 7) การปูสองแบบจะถือว่าต่างกันก็ต่อเมื่อมีคู่ช่องที่ติดกันแบบ 4 ทิศคู่ใดคู่หนึ่ง ที่การปูแบบหนึ่งมี tile ปิดครบทั้งคู่ แต่อีกแบบไม่ครบ" },
              {
                t: "example",
                c: [
                  {
                    input: "n = 1",
                    output: "1",
                    explain: "ปูโดมิโนแนวตั้งหนึ่งอันได้แบบเดียว",
                  },
                  {
                    input: "n = 3",
                    output: "5",
                    explain: "มีวิธีปูที่ต่างกันทั้งหมด 5 แบบ (ผสมโดมิโนและโทรมิโนในรูปแบบต่าง ๆ)",
                  },
                  {
                    input: "n = 4",
                    output: "11",
