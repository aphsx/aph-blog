...
รับประกันว่าหลัง reorder แล้ว ทุกเมืองสามารถไปถึงเมือง 0 ได้`,
        ...
        },
        {
          ...
          c: [
            ...
            {
              t: "p",
              c: "ถ้าเก็บแค่ทิศจริง DFS จาก 0 จะไปไม่ทั่ว — เมือง 4 มีแต่ลูกศรชี้เข้า 0 ไม่มีทางออกจาก 0 ไป 4 ในกราฟทิศจริง ทางออก: เก็บกราฟให้เดินได้สองทาง แต่ติดป้ายว่าทางไหนเป็นทิศจริง ของที่ต้องประกอบมีสามชิ้น",
            },
            {
              t: "ul",
              c: [
                "graph (dict → list ของคู่) — เพื่อนบ้านพร้อมป้าย cost",
                "visited (set) — จำเมืองที่เดินแล้ว กันวนบนต้นไม้ที่เราทำเป็นสองทิศ",
                "changes — สะสม cost 1 ที่เจอตอนเดินออกจาก 0",
              ],
            },

            {
              t: "p",
              c: "ชิ้นที่ 1 · ขอดูหน้าตา graph หลังสร้างเสร็จทั้งก้อนก่อน — แต่ละคู่คือ (เมืองปลาย, cost) โดย cost 1 = ทิศจริงตามโจทย์, cost 0 = ทางที่เราเติมเพื่อเดินย้อน",
            },
            {
              t: "codeout",
              lang: "python",
...
ทั้งก้อน: {0: [(1, 1), (4, 0)], 1: [(0, 0), (3, 1)], 2: [(3, 1)], 3: [(1, 0), (2, 0)], 4: [(0, 1), (5, 1)], 5: [(4, 0)]}`,
            ...
            {
              t: "p",
              c: "ชิ้นที่ 2 · defaultdict(list) ตอนยังว่าง แล้วใส่ connections คู่แรก [0, 1] หนึ่งคู่ — ทิศจริงใส่ cost 1 ทางกลับใส่ cost 0",
            },
            {
              t: "codeout",
              lang: "python",
...
a, b = 0, 1
graph[a].append((b, 1))  # ทิศจริง 0→1
...
print({k: graph[k] for k in sorted(graph)})`,
            ...
            {
              t: "p",
              c: "ชิ้นที่ 4 · DFS จาก 0 บนกราฟนี้: mark แล้ววนเพื่อนบ้าน ตัวไหนยังไม่ visited ให้บวก cost เข้า changes แล้วดิ่งต่อ — บน Example 1 ตอน city=0 คู่แรก nxt=1 cost=1 จึง changes กลายเป็น 1 ก่อนลง dfs(1)",
            },
            {
              t: "code",
              lang: "python",
              label: "เดินออกจาก 0 แล้วสะสม cost",
              c: `visited: set[int] = set()
changes = 0
...
dfs(0)`,
            },
            {
              t: "ul",
              c: [
                "visited สมาชิก = เมืองที่ dfs เข้าแล้ว",
                "changes += cost · cost เป็น 0 หรือ 1 เท่านั้น บวก 0 เท่ากับไม่นับ",
                "เริ่ม dfs(0) เพราะเราเดินออกจากเมืองหลวงตามขั้นที่ 2",
              ],
            },
            {
              t: "p",
              c: "ประกอบสามชิ้น: สร้าง graph ตามชิ้นที่ 3 แล้ว dfs ตามชิ้นที่ 4 ค่า changes สุดท้ายคือคำตอบ",
            },
    ...
    def minReorder(self, n: int, connections: List[List[int]]) -> int:
        ...
        return dfs(0)`,
            ...
            { t: "h3", c: "อ่านโค้ดทีละส่วน" },
            {
              t: "ol",
              c: [
                "คู่แรก [0,1] ทำให้ graph[0] ได้ (1,1) และ graph[1] ได้ (0,0) ตามชิ้นที่ 2",
                "หลังวนครบ Example 1 ได้ dict เดียวกับชิ้นที่ 1",
                "dfs(0) เจอ (1,1) ก่อน → changes = 1 แล้วลง 1 · จาก 1 เจอ (3,1) → changes = 2",
                "จาก 3 ไป 2 ด้วย cost 0 ไม่บวก · ถอยกลับมาที่ 0 แล้วไป 4 ด้วย cost 0 · จาก 4 ไป 5 ด้วย cost 1 → changes = 3",
              ],
            },
            { t: "h3", c: "ต้นทุน" },
            {
              t: "p",
              c: "เวลา O(n) เยี่ยมทุกเมืองและทุกเส้นหนึ่งครั้ง (มี n−1 เส้น แต่เก็บสองทิศ) · หน่วยความจำ O(n) จากกราฟ visited และ call stack",
            },
          ],
        },
      ],
