...
export const backtrackingPages: Record<string, Page> = {
  "lc75-intro-backtracking": {
    ...
    title: { th: "Backtracking — พื้นฐาน & แนวคิด", en: "" },
    lead: { th: "เทคนิคไล่ลองทุกทางเลือกแบบมีระบบ ลงลึกไปทีละก้าว แล้วถอยกลับ (undo) เพื่อลองทางอื่น", en: "" },
    group: "LeetCode 75",
    blocks: {
      th: [
  ...
  "lc75-p57": {
    slug: "lc75-p57",
    title: { th: "ข้อ 57 · LC17 Letter Combinations of a Phone Number (ตัวอักษรจากเบอร์โทร) 🟡", en: "" },
    lead: { th: "map (แปลง) ตัวเลขบนแป้นโทรศัพท์เป็นทุก combination ของตัวอักษรที่กดได้ ด้วย backtracking ทีละ digit", en: "" },
    group: "LeetCode 75",
    blocks: {
      th: [
              { t: "p", c: "โจทย์ (LC17): กำหนด string (สตริง) ที่ประกอบด้วย digit (หลักตัวเลข) ตั้งแต่ 2 ถึง 9 มา ให้ return ทุก letter combination ที่เป็นไปได้ที่ตัวเลขนั้นอาจหมายถึง (เรียงลำดับคำตอบแบบใดก็ได้) โดย mapping ของแต่ละ digit ไปตัวอักษรเป็นแบบเดียวกับปุ่มโทรศัพท์: 2=abc, 3=def, 4=ghi, 5=jkl, 6=mno, 7=pqrs, 8=tuv, 9=wxyz (เลข 1 ไม่มีตัวอักษรกำกับ)" },
              {
                t: "example",
                c: [
                  {
                    ...
                    explain: "เลข 2 ให้ a/b/c เลข 3 ให้ d/e/f จับคู่กันครบทุกแบบ",
                  },
                  {
                    input: 'digits = "2"',
                    output: "['a','b','c']",
                  },
                  {
                    input: 'digits = ""',
                    output: "[]",
                    explain: "สตริงว่าง ไม่มีอะไรให้กดเลย",
                  },
                ],
              },
              {
...
[]` },
  ...
  "lc75-p58": {
    slug: "lc75-p58",
    title: { th: "ข้อ 58 · LC216 Combination Sum III (ผลรวมชุดค่า III) 🟡", en: "" },
    lead: { th: "หาทุก combination ของเลข k ตัวจาก 1-9 ที่ไม่ซ้ำและ sum (รวม) ได้ n ด้วย backtracking พร้อม pruning (ตัดกิ่ง)", en: "" },
    group: "LeetCode 75",
    blocks: {
      th: [
              { t: "p", c: "โจทย์ (LC216): กำหนดจำนวนเต็ม k กับ n มา ให้หาทุก combination ที่ใช้ได้ของเลขจำนวนเต็ม k ตัว ซึ่ง sum แล้วได้เท่ากับ n โดยมีเงื่อนไขว่าใช้ได้เฉพาะเลข 1 ถึง 9 เท่านั้น และแต่ละเลขใช้ได้อย่างมากหนึ่งครั้ง ให้ return list ของทุก combination ที่ใช้ได้ (ห้ามมี combination ซ้ำกัน ลำดับใน list ไม่สำคัญ)" },
              {
                t: "example",
                c: [
                  {
                    input: "k = 3, n = 7",
                    output: "[[1,2,4]]",
                    explain: "1 + 2 + 4 = 7 และไม่มี combination อื่นที่ใช้ได้อีกแล้ว",
                  },
                  {
                    input: "k = 3, n = 9",
                    output: "[[1,2,6],[1,3,5],[2,3,4]]",
                    explain: "แต่ละชุดเรียงน้อยไปมาก sum ได้ 9 ทั้งหมด และไม่ซ้ำกัน",
                  },
                  {
                    input: "k = 4, n = 1",
                    output: "[]",
                    explain: "เลือกเลข 4 ตัวไม่ซ้ำจาก 1-9 ผลรวมน้อยที่สุดที่ทำได้คือ 1+2+3+4 = 10 ซึ่งมากกว่า 1 อยู่แล้ว จึงไม่มี combination ที่ใช้ได้",
                  },
                ],
              },
              {
