...
''`,
  ...
  "lc75-p07": {
    ...
    group: "LeetCode 75",
    blocks: [
      { t: "p", c: "ให้ array (ลิสต์) nums จง return array answer โดยที่ answer[i] คือ ผลคูณของทุก element (สมาชิก) ยกเว้นตัวที่ i" },
      { t: "p", c: "เงื่อนไขสำคัญสองข้อที่ทำให้โจทย์นี้สนุก: ห้ามใช้การหาร (division) และต้องทำใน O(n)" },
      {
        t: "example",
        c: [
          {
          ...
          },
          {
            ...
            output: "[0,0,9,0,0]",
            explain: "มี 0 อยู่หนึ่งตัว → ทุกตำแหน่งที่ไม่ใช่ตำแหน่งของ 0 จะได้ 0\nตำแหน่งของ 0 เอง ได้ (-1)×1×(-3)×3 = 9",
          },
          {
            input: "nums = [0,0,2]",
            output: "[0,0,0]",
            explain: "มี 0 สองตัว → ทุกตำแหน่งได้ 0 หมด (เพราะยังเหลือ 0 อีกตัวคูณอยู่) — เคสนี้พังทันทีถ้าคิดวิธี \"หารด้วยตัวเอง\"",
          },
        ],
      },
      {
        ...
        c: [
          ...
          "ท้าทาย: ทำให้ใช้ extra space เป็น O(1) (ไม่นับ output array)",
        ],
      },
      { t: "callout", title: "โจทย์นี้ถามอะไรจริง ๆ", c: "answer[i] = (ผลคูณของทุกตัวทางซ้ายของ i) × (ผลคูณของทุกตัวทางขวาของ i) — พอเขียนออกมาแบบนี้ วิธีทำก็ชัดเลย: คำนวณผลคูณสะสมจากซ้ายรอบหนึ่ง แล้วจากขวาอีกรอบหนึ่ง" },
    ...
    # เขียนโค้ดของคุณที่นี่ (ห้ามใช้เครื่องหมาย / )
    pass
            ...
            c: [
              {
                t: "ol",
                c: [
                  "answer[i] แยกออกเป็นผลคูณสองก้อนได้ไหม — ก้อนซ้ายของ i และก้อนขวาของ i?",
                  "ถ้ารู้ผลคูณของทุกตัวทางซ้ายของ i แล้ว จะหาผลคูณของทุกตัวทางซ้ายของ i+1 ได้ยังไง โดยไม่ต้องคูณใหม่ทั้งหมด?",
                  "จากคำถามข้อ 2 คุณต้องกวาด array กี่รอบ และกวาดทิศไหน?",
                  "ทำไมโจทย์ต้องห้ามการหาร — ลองคิดว่าถ้ามี 0 ใน array วิธี \"ผลคูณทั้งหมด ÷ nums[i]\" จะเกิดอะไรขึ้น?",
                ],
              },
              { t: "p", c: "ข้อ 2 คือหัวใจ — คำว่า \"ไม่ต้องคูณใหม่ทั้งหมด\" หมายถึงเอาค่าเดิมมาต่อยอด (running product)" },
            ],
          },
          {
            title: "💡 ใบ้ขั้น 2 — เทคนิคที่ต้องใช้ และใช้ทำไม",
            c: [
              { t: "p", c: "ใช้ prefix product คู่กับ suffix product (ผลคูณสะสมจากซ้าย และจากขวา)" },
              {
                t: "ul",
                c: [
                  "รอบที่ 1 — กวาดซ้าย→ขวา เก็บ prefix ลงใน answer[i] โดย ใส่ก่อนแล้วค่อยคูณตัวเอง เข้าไป (ลำดับนี้สำคัญ เพราะทำให้ answer[i] ไม่มีตัวเองอยู่ในผลคูณ)",
                  "รอบที่ 2 — กวาดขวา→ซ้าย คูณ suffix เข้าไปทับ answer[i] ด้วยตรรกะเดียวกัน",
                  "ผลคือ answer[i] = prefix × suffix = ผลคูณทุกตัวยกเว้นตัวเอง โดยไม่ต้องหารเลย",
                ],
              },
              { t: "p", c: "ทำไมได้ O(1) extra space: เราใช้ answer array (ซึ่งเป็น output ที่ต้องสร้างอยู่แล้ว จึงไม่นับ) เป็นที่เก็บ prefix ชั่วคราว แล้วใช้ตัวแปรเดียว (suffix) วิ่งย้อนกลับ ไม่ต้องมี array ที่สอง" },
              { t: "callout", title: "อย่าทำสิ่งนี้", c: "อย่าใช้วิธี \"คูณทุกตัวได้ total แล้ว answer[i] = total // nums[i]\" — โจทย์ห้ามหาร และมันพังจริงเมื่อมี 0 (ZeroDivisionError หรือคำตอบผิดถ้ามี 0 สองตัว) และอย่าใช้ nested loop คูณใหม่ทุกตำแหน่งเพราะเป็น O(n²) กับ n = 10^5" },
            ],
          },
          {
            title: "💡 ใบ้ขั้น 3 — โครงโค้ด (pseudocode) มีช่องว่างให้เติม",
...
for i in range(n):              # รอบ 1: ซ้าย -> ขวา
    answer[i] = ______          # (1) ใส่ผลคูณฝั่งซ้ายก่อน
    prefix ______               # (2) แล้วค่อยคูณตัวเองเข้าไป

suffix = 1
for i in range(______):         # (3) รอบ 2: ขวา -> ซ้าย (เขียน range ยังไง?)
    answer[i] ______            # (4) คูณ suffix เข้าไปทับ
    suffix ______               # (5)

return answer`,
              },
              { t: "p", c: "ลำดับในช่อง (1)(2) สลับกันไม่ได้เด็ดขาด — ถ้าคูณตัวเองเข้า prefix ก่อนแล้วค่อยใส่ answer[i] ผลคูณจะมีตัวเองติดไปด้วย ซึ่งผิดโจทย์ทันที" },
            ],
          },
        ],
      },
      ...
      { t: "h2", c: "ไล่ทีละสเต็ปด้วยมือ (dry run)" },
      { t: "p", c: "ไล่ Example 1: nums = [1, 2, 3, 4] — รอบที่ 1 (prefix จากซ้าย)" },
      {
        t: "table",
      ...
      { t: "p", c: "รอบที่ 2 (suffix จากขวา) — คูณทับลงไปบน answer เดิม" },
      {
        t: "table",
        head: ["i", "answer[i] เดิม (prefix)", "suffix ก่อนคูณ", "answer[i] ใหม่", "suffix หลังคูณ nums[i]"],
        rows: [
          ["3", "6", "1", "6 × 1 = 6", "1 × 4 = 4"],
          ["2", "2", "4", "2 × 4 = 8", "4 × 3 = 12"],
      ...
      { t: "p", c: "ได้ answer = [24, 12, 8, 6] ตรงกับ Example 1 — และไม่มีการหารแม้แต่ครั้งเดียว" },

      {
        t: "solution",
        summary: "🔓 เปิดเฉลยเต็ม (ลองเองก่อนนะ)",
        c: [
          { t: "p", c: "ไอเดียหนึ่งบรรทัด: กวาดสองรอบ — รอบแรกใส่ผลคูณฝั่งซ้ายลง answer รอบสองคูณผลคูณฝั่งขวาเข้าไปทับ" },
          {
            t: "codeout",
            lang: "python",
            label: "เฉลย (Python)",
    ...
    for i in range(n):
        answer[i] = prefix           # (3) ใส่ผลคูณฝั่งซ้ายลงไปก่อน (ยังไม่รวมตัวเอง)
        prefix *= nums[i]
    suffix = 1                       # (4) ผลคูณของทุกตัว "ทางขวา" ของ i
    for i in range(n - 1, -1, -1):
        answer[i] *= suffix          # (5) คูณฝั่งขวาเข้าไปทับ
        suffix *= nums[i]
    return answer                    # (6)
...
[3, 2]`,
          },
          {
            ...
            head: ["บรรทัด", "โค้ด", "ทำอะไร / ทำไมต้องมี"],
            rows: [
              ["(1)", "answer = [1] * n", "เริ่มด้วย 1 ทุกช่องเพราะ 1 เป็นเอกลักษณ์ของการคูณ (คูณเข้าไปแล้วไม่เปลี่ยนค่า) — array นี้คือ output จึงไม่นับเป็น extra space"],
              ["(2)", "prefix = 1", "ตัวแปรสะสมผลคูณจากซ้าย ณ ตอนที่อยู่ที่ i มันคือผลคูณของ nums[0..i-1] พอดี"],
              ["(3)", "answer[i] = prefix แล้วค่อย prefix *= nums[i]", "ลำดับนี้คือหัวใจทั้งข้อ — ใส่ค่าก่อนคูณตัวเอง ทำให้ answer[i] ไม่มี nums[i] อยู่ในผลคูณ ถ้าสลับสองบรรทัดนี้คำตอบจะผิดทั้งหมด"],
              ["(4)", "suffix = 1", "ตัวแปรสะสมผลคูณจากขวา ใช้ตัวแปรเดียวจึงเป็น O(1) extra space ไม่ต้องสร้าง array suffix แยก"],
              ["(5)", "answer[i] *= suffix แล้วค่อย suffix *= nums[i]", "คูณทับด้วยตรรกะเดียวกันแต่กลับทิศ — ตอนนี้ answer[i] = ซ้าย × ขวา = คำตอบที่ต้องการ"],
              ["(6)", "return answer", "ไม่มีการหารเลยตลอดทั้งฟังก์ชัน จึงรอดทุกกรณีที่มี 0"],
            ],
          },
          { t: "p", c: "ทำไมมันถูกต้อง: invariant ของรอบแรกคือ \"ก่อนเข้ารอบที่ i ตัวแปร prefix เท่ากับผลคูณของ nums[0..i-1]\" (ตอน i = 0 ผลคูณของเซตว่างคือ 1 ซึ่งถูกต้อง) ส่วนรอบสองคือกระจกเงาของรอบแรก เมื่อจบทั้งสองรอบ answer[i] = (ผลคูณซ้าย) × (ผลคูณขวา) ตามนิยามของโจทย์เป๊ะ" },
          {
            t: "table",
            head: ["วิธี", "Time", "Space (ไม่นับ output)", "หมายเหตุ"],
            rows: [
              ["nested loop คูณใหม่ทุกตำแหน่ง", "O(n²) ❌", "O(1)", "n = 10^5 → TLE"],
              ["ผลคูณทั้งหมด ÷ nums[i]", "O(n)", "O(1)", "ผิดกติกา และพังเมื่อมี 0 ❌"],
              ["สร้าง array prefix + array suffix", "O(n) ✅", "O(n)", "ถูกและเข้าใจง่าย แต่ยังไม่ผ่าน challenge"],
              ["prefix แล้ว suffix ทับ (เฉลยนี้)", "O(n) ✅", "O(1) ✅", "ผ่าน challenge ของโจทย์"],
            ],
          },
          {
            t: "details",
            summary: "ทำไมวิธีหารถึงใช้ไม่ได้ — ดูของจริง",
            c: [
              {
                t: "codeout",
                lang: "python",
                label: "เทียบวิธีหารกับเฉลย",
...
def with_division(nums):
    ...
    for x in nums:
        total *= x
    return [total // x for x in nums]      # ระเบิดทันทีถ้ามี 0
...
print(product_except_self([-1, 1, 0, -3, 3]), "<- วิธีไม่ใช้การหารรอดทุกกรณี")`,
                out: `[24, 12, 8, 6]
ZeroDivisionError: integer division or modulo by zero
[0, 0, 9, 0, 0] <- วิธีไม่ใช้การหารรอดทุกกรณี`,
              },
              { t: "p", c: "จะแก้วิธีหารด้วยการนับจำนวน 0 แล้วแยกเคสก็ได้ (0 ตัว / 1 ตัว / มากกว่า 1 ตัว) แต่โค้ดจะยาวกว่าและโจทย์ห้ามหารอยู่แล้ว — prefix/suffix จึงเป็นคำตอบที่ถูกต้องและสั้นกว่า" },
            ],
          },
        ],
      },

      { t: "callout", title: "💡 สรุป pattern", c: "prefix / suffix accumulation: เมื่อคำตอบของตำแหน่ง i ขึ้นกับ \"ทุกอย่างทางซ้าย\" และ \"ทุกอย่างทางขวา\" ให้กวาดสองรอบสวนทางกันแล้วรวมผล — ท่านี้ใช้ได้ทั้งผลคูณ ผลรวม ค่ามากสุด และเป็นรากของหมวด Prefix Sum ที่จะเจอถัดไป" },
